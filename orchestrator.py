...
def setup_mcp(project_path: Path, preset: str = None, interactive: bool = True) -> Optional[str]:
    """Run MCP setup and return path to config file."""
    mcp_script = Path(__file__).parent / "mcp-setup.py"
    
    # Ensure absolute paths
    project_path = Path(project_path).expanduser().resolve()
    config_file = project_path / "mcp-config.json"
    
    if not mcp_script.exists():
        print_status("MCP setup script not found, skipping MCP configuration", "warning")
        return None
    
    cmd = ["python3", str(mcp_script), "--project", str(project_path), "--output", str(config_file)]
    
    if preset:
        cmd.extend(["--preset", preset])
        print_status(f"Setting up MCP with preset: {preset}", "working")
        result = subprocess.run(cmd, cwd=str(project_path))
    elif interactive:
        print_status("Starting interactive MCP setup...", "working")
        result = subprocess.run(cmd, cwd=str(project_path))
    else:
        # Smart mode
        cmd.append("--smart")
        print_status("Running smart MCP detection...", "working")
        result = subprocess.run(cmd, cwd=str(project_path))
    
    if config_file.exists():
        abs_path = str(config_file.resolve())
        print_status(f"MCP config created: {abs_path}", "success")
        return abs_path
...
2. Store large outputs as artifacts (don't paste into context)
3. Run verification after implementation
4. If tests pass, mark feature as passes: true in feature_list.json
5. Capture what worked: .agent/commands.sh success "{feature.get('id')}" "description"
6. If something fails, capture it: .agent/commands.sh failure "{feature.get('id')}" "description"
7. Commit with: git commit -m "session: completed {feature.get('id')}"

If you get stuck after 3 attempts, mark the feature as blocked and move on."""
...
def run_claude_code_interactive(
    ...
    if mcp_config:
        mcp_path = Path(mcp_config).expanduser().resolve()
        if mcp_path.exists():
            cmd.append("--mcp-config")
            cmd.append(str(mcp_path))
            print_status(f"MCP config: {mcp_path}", "info")
        else:
            print_status(f"MCP config not found: {mcp_path}, skipping", "warning")
    
    # Use -p flag for prompt (safer than positional)
    cmd.append("-p")
    cmd.append(f"Read and execute the instructions in {prompt_file}")
    
    print_status(f"Starting interactive Claude Code session ({model})...", "working")
...
def orchestrate_new_project(info: Dict[str, Any], mcp_config: str = None):
    ...
    if info.get('mcp_setup'):
        mcp_config = setup_mcp(project_path, preset=info.get('mcp_preset'), interactive=True)
    elif info.get('mcp_preset'):
        mcp_config = setup_mcp(project_path, preset=info['mcp_preset'], interactive=False)
    
    # Session 1: Initialize project
    print_header("Session 1: Project Initialization")
    
    init_prompt = build_init_prompt(info)
    result = run_claude_code_interactive(project_path, init_prompt, info.get('model', DEFAULT_MODEL), mcp_config)
    ...
    orchestrate_implementation(project_path, info.get('model', DEFAULT_MODEL), start_session=2, mcp_config=mcp_config)

def orchestrate_implementation(project_path: Path, model: str = DEFAULT_MODEL, start_session: int = 1, mcp_config: str = None):
    ...
    while session_num <= MAX_SESSIONS:
        ...
        result = run_claude_code_interactive(project_path, prompt, model, mcp_config)
...
def orchestrate_continue(project_path: Path, model: str = DEFAULT_MODEL, mcp_config: str = None):
    ...
    if not mcp_config and (project_path / "mcp-config.json").exists():
        mcp_config = str(project_path / "mcp-config.json")
        print_status(f"Found existing MCP config: {mcp_config}", "info")
    ...
    orchestrate_implementation(project_path, model, start_session, mcp_config)
...
Examples:
  ...
  python orchestrator.py --model opus        # Use Opus model
    ...
    parser.add_argument("--mcp", action="store_true", help="Enable MCP and run interactive setup")
    ...
    if args.cont or args.project:
        ...
        if not project_path.exists():
            ...
            sys.exit(1)
        orchestrate_continue(project_path, args.model, args.mcp_config)
        return
    ...
    if args.new:
        ...
        info['mcp_setup'] = args.mcp
        info['mcp_preset'] = args.mcp_preset
        orchestrate_new_project(info, args.mcp_config)
        return
