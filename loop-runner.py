...
def run_session(project_path: Path, session_num: int, model: str, mcp_config: str = None) -> bool:
    ...
    # Build the prompt with explicit test requirements and subagent usage
...
- Use the subagents for review and verification"""
...
def main():
    ...
    parser.add_argument("--interactive", "-i", action="store_true", help="Run interactively (no --print)")
...
6. Use @feature-verifier to verify end-to-end
7. Only mark passes: true if tests pass
8. Commit: git commit -m "session: completed {feature_id}"
...
Do NOT skip running tests!"""
                ...
                if args.mcp_config:
                    mcp_path = Path(args.mcp_config).expanduser().resolve()
                    if mcp_path.exists():
            ...
            try:
                run_session(project_path, session, args.model, args.mcp_config)
            except subprocess.TimeoutExpired:
