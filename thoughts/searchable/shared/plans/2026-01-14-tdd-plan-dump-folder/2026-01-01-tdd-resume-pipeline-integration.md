...
- [x] Unit tests pass: `python -m pytest planning_pipeline/tests/test_checkpoint_manager.py -v`
- [x] Type checking passes: `python -m mypy planning_pipeline/checkpoint_manager.py`
- [x] Module imports work: `python -c "from planning_pipeline import detect_resumable_checkpoint"`

#### Manual Verification:
- [x] `detect_resumable_checkpoint()` finds existing checkpoint in `.workflow-checkpoints/`
- [x] Age calculation works correctly for checkpoint timestamps
- [x] Cleanup functions delete correct files
...
#### Automated Verification:
- [x] Unit tests pass: `python -m pytest planning_pipeline/tests/test_helpers.py -v`
- [x] Unit tests pass: `python -m pytest planning_pipeline/tests/test_checkpoints.py -v`
- [x] Module imports work: `python -c "from planning_pipeline import discover_thoughts_files, prompt_file_selection"`

#### Manual Verification:
- [x] `discover_thoughts_files()` finds today's research files
- [x] `discover_thoughts_files()` with `days_back=7` finds last week's files
- [x] Files are sorted alphabetically
- [x] `prompt_file_selection()` displays menu correctly
- [x] Cleanup menu options all work as expected
...
#### Automated Verification:
- [x] `python planning_orchestrator.py --help` shows new arguments
- [x] `python planning_orchestrator.py --resume --help` shows resume options
- [x] Existing tests pass: `python -m pytest planning_pipeline/tests/test_orchestrator.py -v`

#### Manual Verification:
- [x] `--resume` auto-detects from existing checkpoint
- [x] `--resume` without checkpoint triggers file selection
- [x] `--resume --research-path <path>` skips file selection
- [x] Resume from planning runs planning -> decomposition -> beads
- [x] Resume from decomposition runs decomposition -> beads
- [x] Checkpoint is deleted after successful resume
...
#### Automated Verification:
- [x] Pipeline writes checkpoint on failure: `ls .workflow-checkpoints/`
- [x] Checkpoint contains absolute paths
- [x] Checkpoint is deleted after successful completion
- [x] Deprecation warning appears when running `resume_pipeline.py`

#### Manual Verification:
- [x] Interrupt pipeline during planning -> checkpoint created
- [x] Resume with `--resume` -> uses checkpoint
- [x] Complete pipeline run -> no checkpoint files remain
- [x] Old checkpoints > 30 days trigger cleanup prompt
